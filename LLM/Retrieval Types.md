# A Deep Dive into Retriever

A retriever is an interface that returns documents given an unstructured query. It is more general than a vector store. A retriever does not need to be able to store documents, only to return (or retrieve) them. Vector stores can be used as the backbone of a retriever, but there are other types of retrievers as well.

Retrievers accept a string query as input and return a list of Document's as output.

## Define the persistent directory and embeddings model
```py
...
The vector store (Chroma) is loaded from the persistent directory with the embedding function.
...
The user’s query, “How did Juliet die?” is used as an example to showcase how different retrieval methods work.
